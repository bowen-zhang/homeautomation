...
class MongoStorage(pattern.Singleton):
  def __init__(self, database_name, client=None):
    self._snapshots = mongo_storage.ProtobufMongoStorage(
        proto_cls=weather_pb2.Snapshot,
    ...
    self._locations = mongo_storage.ProtobufMongoStorage(
        proto_cls=common_pb2.Location,
        database=database_name,
  ...
  def create_indexes(self):
    self._snapshots.create_indexes()
    self._locations.create_indexes()
